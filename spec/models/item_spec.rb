...
RSpec.describe Item, type: :model do
  ...
  describe 'ユーザー新規登録' do
    ...
    context '商品出品できない時' do
      ...
      it "nameが存在しない場合は出品できないこと" do
        @item.name = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Name can't be blank")
      end

      it "explanatが存在しない場合は出品できないこと" do
        @item.explanat = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Explanat can't be blank")
      end

      it "priceが存在しない場合は出品できないこと" do
        @item.price = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Price can't be blank")
      end

      it "category_idが存在しない場合は出品できないこと" do
        @item.category_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Category_id can't be blank")
      end

      it "status_idが存在しない場合は出品できないこと" do
        @item.status_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Status_id can't be blank")
      end

      it "derivery_charge_idが存在しない場合は出品できないこと" do
        @item.derivery_charge_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Derivery_charge_id can't be blank")
      end

      it "sender_area_idが存在しない場合は出品できないこと" do
        @item.sender_area_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Sender_area_id can't be blank")
      end

      it "daystosend_idが存在しない場合は出品できないこと" do
        @item.daystosend_id = nil
        @item.valid?
        expect(@item.errors.full_messages).to include("Daystosend_id can't be blank")
      end
